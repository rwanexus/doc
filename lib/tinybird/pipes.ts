...
const tb = TINYBIRD_ENABLED 
  ? new Tinybird({ token: process.env.TINYBIRD_TOKEN! })
  : null;

// Helper to create a mock pipe that returns empty data
const createMockPipe = <T>() => {
  return async (_params: any): Promise<{ data: T[] }> => {
    return { data: [] };
  };
};
...
export const getTotalAvgPageDuration = tb 
  ...
  : createMockPipe<{ versionNumber: number; pageNumber: string; avg_duration: number }>();
...
export const getViewPageDuration = tb
  ...
  : createMockPipe<{ pageNumber: string; sum_duration: number }>();
...
export const getViewCompletionStats = tb
  ...
  : createMockPipe<{ viewId: string; versionNumber: number; pages_viewed: number }>();
...
export const getTotalDocumentDuration = tb
  ...
  : createMockPipe<{ sum_duration: number }>();
...
export const getTotalLinkDuration = tb
  ...
  : createMockPipe<{ sum_duration: number; view_count: number }>();
...
export const getViewUserAgent_v2 = tb
  ...
  : createMockPipe<{ country: string; city: string; browser: string; os: string; device: string }>();
...
export const getTotalDataroomDuration = tb
  ...
  : createMockPipe<{ viewId: string; sum_duration: number }>();
...
export const getTotalTeamDuration = tb
  ...
  : createMockPipe<{ total_duration: number; unique_countries: string[] }>();
